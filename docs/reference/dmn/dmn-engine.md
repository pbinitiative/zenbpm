...
- A `requiredDecision` is evaluated recursively **with the original evaluation input**, so a required decision never sees the results of its siblings. Its result is then merged into the requiring decision's variable context under the required decision's `id`. The `name` attribute is display-only.
