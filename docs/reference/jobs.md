...
**Lock deadline.** Every `WaitingJob` carries `lock_until`, the unix millisecond on the clock of the partition leader at which its lock lapses. `lock_until - now` on the client is an *estimate* of the remaining time: it is exact only as far as the two clocks agree, and it does not see the time the delivery spent in transit. Keep the clocks synchronised (NTP), and renew with a safety margin, at the latest at half the remaining time, rather than just before the deadline. When the clocks cannot be trusted, count from the moment of receipt with the lock duration you subscribed with, which needs no clock comparison at all. The Go client exposes the deadline as `job.GetLockUntil()`.
