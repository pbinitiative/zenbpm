...
func TestReadyTargetUnsubscribeKeepsOtherSubscribers(t *testing.T) {
	rt := NewReadyTarget[int]()
	ch2 := rt.Subscribe(2)
	ch5 := rt.Subscribe(5)
	ch8 := rt.Subscribe(8)
	...
	case <-ch8:
	default:
		t.Fatal("target 8 was not signalled")
	}
	select {
	case <-ch5:
		t.Fatal("unsubscribed target 5 should not be signalled")
	default:
	}
}
