...
func createCallActivityProcessInstance(t *testing.T, callActivityFilename string) runtime.ProcessInstance {
	...
	_, err := bpmnEngine.LoadFromFile("./test-cases/call_activity_with_error_boundary_event_child_process.bpmn")
	assert.NoError(t, err)

	processDefinition, err := bpmnEngine.LoadFromFile("./test-cases/" + callActivityFilename)
...
func createProcessInstance(t *testing.T, filename string) runtime.ProcessInstance {
	...
	processDefinition, err := bpmnEngine.LoadFromFile("./test-cases/" + filename)
	require.NoError(t, err)
