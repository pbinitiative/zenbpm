...
func TestJobFailOnNestedSubProcessBoundaryIsCaughtInAncestor(t *testing.T) {
	createdProcessInstance := createProcessInstance(t, "sub_process/subprocess_nested_with_error_boundery_event.bpmn")

	var outerSubProcess runtime.ProcessInstance
	require.Eventually(t, func() bool {
		var found bool
		outerSubProcess, found = findChildProcessInstanceByParentAndType(createdProcessInstance.ProcessInstance().Key, runtime.ProcessTypeSubProcess)
		return found
	}, time.Second, 20*time.Millisecond)

	var middleSubProcess runtime.ProcessInstance
	require.Eventually(t, func() bool {
		var found bool
		middleSubProcess, found = findChildProcessInstanceByParentAndType(outerSubProcess.ProcessInstance().Key, runtime.ProcessTypeSubProcess)
		return found
	}, time.Second, 20*time.Millisecond)

	var innerSubProcess runtime.ProcessInstance
	require.Eventually(t, func() bool {
		var found bool
		innerSubProcess, found = findChildProcessInstanceByParentAndType(middleSubProcess.ProcessInstance().Key, runtime.ProcessTypeSubProcess)
		return found
	}, time.Second, 20*time.Millisecond)

	var job runtime.Job
	require.Eventually(t, func() bool {
		job = findJobForProcessInstance(innerSubProcess.ProcessInstance().Key, "service_task")
		return job.Key != 0
	...
	require.Eventually(t, func() bool {
		createdProcessInstance, err = bpmnEngine.persistence.FindProcessInstanceByKey(t.Context(), createdProcessInstance.ProcessInstance().Key)
		require.NoError(t, err)
		return createdProcessInstance.ProcessInstance().State == runtime.ActivityStateCompleted
	}, time.Second, 20*time.Millisecond)

	outerSubProcess, err = bpmnEngine.persistence.FindProcessInstanceByKey(t.Context(), outerSubProcess.ProcessInstance().Key)
	assert.NoError(t, err)
	assert.Equal(t, runtime.ActivityStateCompleted, outerSubProcess.ProcessInstance().State)

	middleSubProcess, err = bpmnEngine.persistence.FindProcessInstanceByKey(t.Context(), middleSubProcess.ProcessInstance().Key)
	assert.NoError(t, err)
	assert.Equal(t, runtime.ActivityStateTerminated, middleSubProcess.ProcessInstance().State)

	innerSubProcess, err = bpmnEngine.persistence.FindProcessInstanceByKey(t.Context(), innerSubProcess.ProcessInstance().Key)
	assert.NoError(t, err)
	assert.Equal(t, runtime.ActivityStateTerminated, innerSubProcess.ProcessInstance().State)

	assertJobState(t, job, runtime.ActivityStateTerminated)
	assertIncidentCount(t, createdProcessInstance, 0)
	assertIncidentCount(t, outerSubProcess, 0)
	assertIncidentCount(t, middleSubProcess, 0)
	assertIncidentCount(t, innerSubProcess, 0)
	assertTokenElementIds(t, createdProcessInstance, []string{"complete"}, nil)
}

func TestJobFailOnMultiInstanceInsideSubProcessWithMatchingErrorBoundaryIsCaught(t *testing.T) {
	processDefinition, err := bpmnEngine.LoadFromFile(t.Context(), "./test-cases/error_events/multi_instance/sequential_multi_instance_in_subprocess_with_error_boundary_event.bpmn")
	require.NoError(t, err)

	createdProcessInstance, err := bpmnEngine.CreateInstanceByKey(t.Context(), processDefinition.Key, map[string]interface{}{
		"testInputCollection": []string{"one", "two"},
	})
	require.NoError(t, err)
	...
	require.Eventually(t, func() bool {
		var found bool
		subProcessInstance, found = findChildProcessInstanceByParentAndType(createdProcessInstance.ProcessInstance().Key, runtime.ProcessTypeSubProcess)
		return found
	}, time.Second, 20*time.Millisecond)

	var multiInstance runtime.ProcessInstance
	require.Eventually(t, func() bool {
		var found bool
		multiInstance, found = findChildProcessInstanceByParentAndType(subProcessInstance.ProcessInstance().Key, runtime.ProcessTypeMultiInstance)
		return found
	}, time.Second, 20*time.Millisecond)

	var job runtime.Job
	require.Eventually(t, func() bool {
		job = findJobForProcessInstance(multiInstance.ProcessInstance().Key, "service_task")
		return job.Key != 0
	...
	subProcessInstance, err = bpmnEngine.persistence.FindProcessInstanceByKey(t.Context(), subProcessInstance.ProcessInstance().Key)
	assert.NoError(t, err)
	assert.Equal(t, runtime.ActivityStateCompleted, subProcessInstance.ProcessInstance().State)

	multiInstance, err = bpmnEngine.persistence.FindProcessInstanceByKey(t.Context(), multiInstance.ProcessInstance().Key)
	assert.NoError(t, err)
	assert.Equal(t, runtime.ActivityStateTerminated, multiInstance.ProcessInstance().State)

	assertJobState(t, job, runtime.ActivityStateTerminated)
	assertIncidentCount(t, createdProcessInstance, 0)
	assertIncidentCount(t, subProcessInstance, 0)
	assertIncidentCount(t, multiInstance, 0)
	assertTokenElementIds(t, createdProcessInstance, []string{"Event_11axlot"}, nil)
}
...
func findChildProcessInstanceByParentAndType(parentProcessInstanceKey int64, processType runtime.ProcessType) (runtime.ProcessInstance, bool) {
	for _, processInstance := range engineStorage.ProcessInstances {
		if processInstance.GetParentProcessInstanceKey() == nil {
			continue
		}
		...
		if processInstance.Type() != processType {
			continue
		}
		return processInstance, true
	}
