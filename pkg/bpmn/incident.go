...
func handleResolveIncident(ctx context.Context, engine *Engine, key int64, err error) (runtime.Incident, runtime.ProcessInstance, error) {
	...
	if err != nil {
		return runtime.Incident{}, nil, errors.Join(newEngineErrorf("failed to find incident with key: %d", key), err)
	}
	...
	if incident.ResolvedAt != nil {
		return runtime.Incident{}, nil, errors.New("incident already resolved")
	}

	instance, err := engine.persistence.FindProcessInstanceByKey(ctx, incident.ProcessInstanceKey)
	if err != nil {
		return runtime.Incident{}, nil, errors.Join(newEngineErrorf("failed to find process instance with key: %d", incident.ProcessInstanceKey), err)
	}

	jobs, err := engine.persistence.FindPendingProcessInstanceJobs(ctx, incident.ProcessInstanceKey)
	if err != nil {
		return runtime.Incident{}, nil, errors.Join(newEngineErrorf("failed to find jobs for token key: %d", incident.Token.Key), err)
	}
	...
	err = batch.Flush(ctx)
	if err != nil {
		return runtime.Incident{}, nil, errors.Join(newEngineErrorf("failed to complete incident with key: %d", key), err)
	}
