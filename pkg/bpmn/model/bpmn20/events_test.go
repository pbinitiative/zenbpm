...
func TestParse_BusinessRuleTaskWithErrorBoundaryEvent(t *testing.T) {
	...
	definitions, err := readBpnmFile(t, "business_rule_task_external_with_error_boundary_event.bpmn")
	if err != nil {
		t.Fatalf("failed to read file: %v", err)
	}
...
func TestParse_BusinessRuleTaskWithCatchallErrorBoundaryEvent(t *testing.T) {
	...
	definitions, err := readBpnmFile(t, "business_rule_task_external_with_catch_all_error_boundary_event.bpmn")
	if err != nil {
		t.Fatalf("failed to read file: %v", err)
	}
	...
	assert.Equal(t, "", errorEventDefinition.ErrorRef)
