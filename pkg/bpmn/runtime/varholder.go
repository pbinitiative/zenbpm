...
// PropagateVariables set a value with given key to the parent VariableHolder
...
// PropagateLocalVariablesToParent propagates local variables to the parent VariableHolder according to mappings
// uses a replaceable evaluateExpression() function eg. engine.evaluateExpression()
func (vh *VariableHolder) PropagateLocalVariablesToParent(mappings []extensions.TIoMapping, evaluateExpression func(expression string, variableContext map[string]interface{}) (interface{}, error)) error {
	if vh.parent == nil {
		return nil
	}
