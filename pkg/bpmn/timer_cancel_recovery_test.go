...
func TestCancelTimerRecovery(t *testing.T) {
	t.Run("failed flush is repolled into timer manager", func(t *testing.T) {
		...
		require.Eventually(t, func() bool {
			return timerManagerHasWaitingTimer(timerManager, timerKey)
		...
		assert.False(t, timerManagerHasWaitingTimer(timerManager, timerKey),
	...
	t.Run("successful flush rejects stale in-flight poll", func(t *testing.T) {
		...
		timerManager.addWaitingTimer(timer)

		batch, err := engine.NewEngineBatchClean()
		require.NoError(t, err)
		require.NoError(t, engine.cancelTimer(t.Context(), &batch, timer))
		require.False(t, timerManagerHasWaitingTimer(timerManager, timer.Key),
		...
		require.NoError(t, timerManager.pollTimers(time.Now().Add(2*time.Hour)))
		timerManager.addWaitingTimer(timer)
		require.True(t, timerManagerHasWaitingTimer(timerManager, timer.Key),
