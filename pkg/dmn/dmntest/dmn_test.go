...
func castTestData(data Data) interface{} {
	...
	case "decimal":
		if strings.Contains(inputValue, ".") {
			// Parse as float64 if it has a decimal point
			num, err := strconv.ParseFloat(inputValue, 64)
			if err != nil {
				panic("wrong type for decimal: " + inputValue)
			}
			return num
		} else {
			// Parse as int64 otherwise
	...
	case "double":
		...
		return num
	case "boolean":
		boolean, err := strconv.ParseBool(inputValue)
		if err != nil {
			panic("wrong type for boolean: " + inputValue)
		}
		return boolean
	case "duration":
		return inputValue
...
func runTest(t *testing.T, testName string, decisionId string, inputData map[string]interface{}, expectedResult any) {
	t.Run(testName, func(t *testing.T) {
		...
		assert.Equal(t, expectedResult, result.DecisionOutput)
	})
}
