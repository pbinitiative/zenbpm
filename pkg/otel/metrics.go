...
type EngineMetrics struct {
	...
	ReconciliationWakeQueueDepth  metric.Int64UpDownCounter
	ReconciliationRetryQueueDepth metric.Int64UpDownCounter
}
...
func NewMetrics(meter metric.Meter) (*EngineMetrics, error) {
	...
	errJoin = errors.Join(errJoin, err)
	reconciliationWakeQueueDepth, err := meter.Int64UpDownCounter("reconciliation_wake_queue_depth", metric.WithDescription("Number of distinct process instances waiting for a recovery wakeup"))
	errJoin = errors.Join(errJoin, err)
	reconciliationRetryQueueDepth, err := meter.Int64UpDownCounter("reconciliation_retry_queue_depth", metric.WithDescription("Number of process instances awaiting a technical recovery retry"))
	errJoin = errors.Join(errJoin, err)
	...
	metrics := EngineMetrics{
		...
		ReconciliationWakeQueueDepth:  reconciliationWakeQueueDepth,
		ReconciliationRetryQueueDepth: reconciliationRetryQueueDepth,
	}
