...
type Storage struct {
	...
	DecisionDefinitions    map[string]map[int64]dmnruntime.DecisionDefinition
	BusinessRuleTasks      map[int64]bpmnruntime.BusinessRuleTask
...
func NewStorage() *Storage {
	return &Storage{
		...
		DecisionDefinitions:    make(map[string]map[int64]dmnruntime.DecisionDefinition),
		BusinessRuleTasks:      make(map[int64]bpmnruntime.BusinessRuleTask),
...
func (mem *Storage) Copy() *Storage {
	...
	for k, v := range mem.BusinessRuleTasks {
		c.BusinessRuleTasks[k] = v
	}
...
func (mem *Storage) GetDecisionDefinitionByIdAndDmnResourceDefinitionKey(ctx context.Context, decisionId string, decisionDefinitionKey int64) (dmnruntime.DecisionDefinition, error) {
	return mem.DecisionDefinitions[decisionId][decisionDefinitionKey], nil
}
...
func (mem *Storage) SaveDecisionDefinition(ctx context.Context, decision dmnruntime.DecisionDefinition) error {
	mem.DecisionDefinitions[decision.Id] = make(map[int64]dmnruntime.DecisionDefinition)
	mem.DecisionDefinitions[decision.Id][decision.DmnResourceDefinitionKey] = decision
	return nil
}
...
var _ storage.BusinessRuleTaskStorageWriter = &Storage{}

func (mem *Storage) SaveBusinessRuleTask(ctx context.Context, result bpmnruntime.BusinessRuleTask) error {
	mem.BusinessRuleTasks[result.Key] = result
	return nil
}

var _ storage.BusinessRuleTaskStorageReader = &Storage{}

func (mem *Storage) FindBusinessRuleTasksByExecutionTokenIds(ctx context.Context, executionTokenIds []int64) ([]bpmnruntime.BusinessRuleTask, error) {
	results := make([]bpmnruntime.BusinessRuleTask, 0)
	for _, result := range mem.BusinessRuleTasks {
		if slices.Contains(executionTokenIds, result.ExecutionTokenKey) {
