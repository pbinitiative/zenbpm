...
type Storage struct {
	...
	BusinessRuleExecutions map[int64]dmnruntime.BusinessRuleExecution
...
func NewStorage() *Storage {
	return &Storage{
		...
		BusinessRuleExecutions: make(map[int64]dmnruntime.BusinessRuleExecution),
...
func (mem *Storage) Copy() *Storage {
	...
	for k, v := range mem.BusinessRuleExecutions {
		c.BusinessRuleExecutions[k] = v
	}
...
var _ storage.BusinessRuleExecutionStorageWriter = &Storage{}

func (mem *Storage) SaveBusinessRuleExecution(ctx context.Context, result dmnruntime.BusinessRuleExecution) error {
	mem.BusinessRuleExecutions[result.Key] = result
	return nil
}

var _ storage.BusinessRuleExecutionStorageReader = &Storage{}

func (mem *Storage) FindBusinessRuleExecutionByKey(ctx context.Context, key int64) (dmnruntime.BusinessRuleExecution, error) {
	for _, result := range mem.BusinessRuleExecutions {
		if key == result.Key {
			return result, nil
		}
	}
	return dmnruntime.BusinessRuleExecution{}, storage.ErrNotFound
}
