...
notify_discord() {
  ...
  content="ZenBPM release ${RELEASE_TAG} ${RELEASE_RESULT}. Workflow: ${WORKFLOW_URL}"
...
case "${1:-}" in
  ...
  prepare-backend-branch) prepare_branch "$BACKEND_REPO" ;;
  prepare-frontend-branch) prepare_branch "$FRONTEND_REPO" ;;
  commit-frontend-release-branch) commit_frontend_release_branch ;;
  tag-backend) create_release_tag "$BACKEND_REPO" ;;
  tag-frontend) create_release_tag "$FRONTEND_REPO" ;;
