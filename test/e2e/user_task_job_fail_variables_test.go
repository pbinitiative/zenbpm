...
func TestUserTaskJobFailVariables(t *testing.T) {
	...
	t.Run("Job fail variables override same-named variables in the local scope", func(t *testing.T) {
		...
		failJobForElementId(t, processInstance.Key, "user_task", nil, map[string]any{"local_variable": "fail_value"})
	...
	t.Run("Job fail variables do not override process variables through output mappings", func(t *testing.T) {
		...
		failJobForElementId(t, processInstance.Key, "user_task", nil, map[string]any{"local_variable": "complete_value"})
